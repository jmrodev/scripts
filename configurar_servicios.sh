...
if [ ! -d "$SCRIPT_DIR" ]; then
    mkdir -p "$SCRIPT_DIR"
    echo "Directorio $SCRIPT_DIR creado."
fi
...
for servicio in "${servicios[@]}"; do
    # Script de inicio
    cat <<EOF > "$SCRIPT_DIR/start_${servicio}.sh"
#!/bin/bash
sudo systemctl start ${servicio}.service
EOF

    # Script de detención
    cat <<EOF > "$SCRIPT_DIR/stop_${servicio}.sh"
#!/bin/bash
sudo systemctl stop ${servicio}.service
EOF

    echo "Scripts para $servicio creados en $SCRIPT_DIR."
done

# Hacer que los scripts sean ejecutables
chmod +x "$SCRIPT_DIR"/start_*.sh "$SCRIPT_DIR"/stop_*.sh
...
chmod +x "$SCRIPT_DIR/gestor_servicios.sh"
